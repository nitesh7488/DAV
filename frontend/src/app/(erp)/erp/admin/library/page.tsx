  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex justify-between items-center">
        ...
        <div className="text-sm text-slate-500">Global Admin Privileges Active</div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <form onSubmit={handleAddBook} className="space-y-4">
            <input type="text" placeholder="Title" required className="w-full border rounded-lg p-2" value={bookForm.title} onChange={(e) => setBookForm({ ...bookForm, title: e.target.value })} />
            <input type="text" placeholder="Author" required className="w-full border rounded-lg p-2" value={bookForm.author} onChange={(e) => setBookForm({ ...bookForm, author: e.target.value })} />
            <input type="text" placeholder="ISBN" required className="w-full border rounded-lg p-2" value={bookForm.isbn} onChange={(e) => setBookForm({ ...bookForm, isbn: e.target.value })} />
            <input type="number" placeholder="Total Copies" required min="1" className="w-full border rounded-lg p-2" value={bookForm.totalCopies} onChange={(e) => setBookForm({ ...bookForm, totalCopies: parseInt(e.target.value) })} />
            <button type="submit" className="w-full bg-indigo-600 text-white rounded-lg p-2 font-semibold hover:bg-indigo-700">Add Book</button>
          </form>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <form onSubmit={handleIssueBook} className="space-y-4">
            <select required className="w-full border rounded-lg p-2" value={issueForm.bookId} onChange={(e) => setIssueForm({ ...issueForm, bookId: e.target.value })}>
            ...
            <input type="text" placeholder="Student ID" required className="w-full border rounded-lg p-2" value={issueForm.studentId} onChange={(e) => setIssueForm({ ...issueForm, studentId: e.target.value })} />
            <p className="text-xs text-slate-500">Policy: Books are issued for a maximum of 7 days.</p>
            <button type="submit" className="w-full bg-emerald-600 text-white rounded-lg p-2 font-semibold hover:bg-emerald-700">Issue Book</button>
          </form>
        </motion.div>
      </div>
